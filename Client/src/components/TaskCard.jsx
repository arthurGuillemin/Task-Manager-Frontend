const TaskCard = ({ task }) => {
  return (
    <div className="border rounded-xl p-6 shadow-md bg-white max-w-md mx-auto">
      ...
      <p className="text-gray-600 mb-4 italic">{task.demandes_precises}</p>
      ...
      <div className="mb-4">
        ...
        <ul className="list-disc list-inside text-gray-600 mt-1">
          {task.checklist.map((item, idx) => (
            <li key={idx} className={item.fait ? "line-through text-green-600" : ""}>
      ...
      {task.remarques && (
        <p className="mt-2 text-yellow-700 italic">{task.remarques}</p>
      )}
    </div>
  );
};
